...
// @property(copy, nonatomic) ImagePickerResult imageResult;

// typedef void (^ImagePickerResult)(id _Nullable result);
...
- (void)pickImagePicker:(int)count result:(FlutterResult)result {
    ...
    [image selectImagePicker:count selectImagePickerBlock:^(NSArray<UIImage *> *images,
        [self selectImageWithResult:images];                                              NSArray<PHAsset *> *assets,BOOL isOriginal){
...
/**
 */
-(void) pickVideoPicker:(FlutterResult)result{
    NSLog(@"%s","pickVideoPicker...");
    int count=1;
    SwiftImagesPicker *image=[[SwiftImagesPicker alloc] init];
    [image selectVideoPicker:count selectVideoPickerBlock:^(NSArray<UIImage *> *images,
                                                            NSArray<PHAsset *> *assets,BOOL isOriginal){
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if ([@"pickImage" isEqualToString:call.method]) {
        ...
        _arguments = call.arguments;
        self.result = result;
        NSNumber *countNumber = [_arguments objectForKey:@"count"];
        if(countNumber==nil){
...
//multi image
- (void)selectMultiImageWithResult:(NSArray<UIImage *> *) results  {
            ...
            if (!originalAsset) {
                ...
                [[PHImageManager defaultManager]
                 ...
                 resultHandler:^(NSData *_Nullable imageData, NSString *_Nullable dataUTI,
                    ...
                    NSString  *savedPath =[self saveImageWithOriginalImageListData:imageData
                                                                             image:image
                                                                          maxWidth:maxWidth
                                                                         maxHeight:maxHeight
 ...
 */
- (void)selectImageWithResult:(NSArray<UIImage *> *) results  {
...
- (NSString *)saveImageWithOriginalImageListData:(NSData *)originalImageData
                                           image:(UIImage *)image
                                        maxWidth:(NSNumber *)maxWidth
                                       maxHeight:(NSNumber *)maxHeight
                                    imageQuality:(NSNumber *)imageQuality {
...
- (NSString *)saveImageListWithPickerInfo:(NSDictionary *)info
                                    image:(UIImage *)image
                             imageQuality:(NSNumber *)imageQuality {
    NSString *savedPath = [FLTImagePickerPhotoAssetUtil saveImageWithPickerInfo:info
                                                                          image:image
